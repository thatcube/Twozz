...
the same arguments), and generate the project through
`./tools/generate-project.sh`. These wrappers participate in the machine-wide
Apple build/cleanup interlock before writing build resources. The desktop
environment this repo is often developed in also injects
`GIT_CONFIG_* (safe.bareRepository=explicit)`, which otherwise makes git refuse
Swift Package Manager's bare dependency repos and breaks `xcodebuild` during
"Resolve Package Graph". `xcbuild.sh` relaxes only that one setting for the
build subprocess. Do not bypass these wrappers with raw `xcodebuild` or
`xcodegen` commands.
...
then `tools/generate-project.sh`.
